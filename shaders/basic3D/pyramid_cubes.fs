...
float scene_dist(vec3 p) {
    ...
    float height_offset = floor(-2.0 - noise31(vec3(cell * noise_scale, t)) * noise_amount);
...
vec3 scene_color(vec3 p, vec3 ro) {
    ...
    vec3 clr = palette(
        ...
        vec3(1.0, 1.0, 1.0),
        color_config.rgb
    ) * color_amount * get_strength(fract(id * 5.0));
...
void main() {
    ...
    vec3 ro = vec3(zoom_x * cos(t), zoom_y, zoom_z * sin(t));
