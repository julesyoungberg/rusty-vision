...
void main() {
    ...
    for (float i = 0.0; i < ITERATIONS; i += 1.0) {
        float angle = time * i * 0.01 + length(st) * PI * (sin(time * 0.05) * 0.5 - 0.7);
        float c = cos(angle);
        float s = sin(angle);
        st *= mat2(c, -s, s, c);
        ...
        float spec1 = texture(sampler2D(spectrum, audio_sampler), vec2(((i * 3)) / (ITERATIONS * 3), 0)).x;
        float spec2 = texture(sampler2D(spectrum, audio_sampler), vec2(((i * 3) + 1) / (ITERATIONS * 3), 0)).x;
        float spec3 = texture(sampler2D(spectrum, audio_sampler), vec2(((i * 3) + 2) / (ITERATIONS * 3), 0)).x;
